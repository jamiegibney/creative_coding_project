        ...
        F: FnMut(usize, &mut [f64]),
    {
        ...
        while already_processed_samples < main_buffer_len {
            ...
            let samples_to_process =
                samples_until_next_window.min(remaining_samples);
            ...
            if samples_to_process == samples_until_next_window {
                for (
                    channel_idx,
                    ((input_ring_buffer, output_ring_buffer), padding_buffer),
                ) in self
                    ...
                    .enumerate()
                {
                    ...
                    callback(channel_idx, &mut self.scratch_buffer);

                    if self.padding > 0 {
                        let padding_to_copy =
                            std::cmp::min(self.padding, block_size);
                        for (scratch_sample, padding_sample) in self
                            .scratch_buffer[..padding_to_copy]
                            .iter_mut()
                    ...
                    add_scratch_to_ring(
                        &self.scratch_buffer, self.current_pos,
                        output_ring_buffer,
                    );
